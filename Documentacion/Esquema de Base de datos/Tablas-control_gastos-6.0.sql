...
insert into category_gasto(cat_description) values('Comida');
...
insert into category_gasto(cat_description) values('Salidas');
insert into category_gasto(cat_description) values('Otro');
...
SELECT p.pa_id, p.usu_id, p.pa_numpago, p.pa_fecha_pagar,p.pa_fecha_pago, p.pa_monto, gasto.ga_description, gasto.ga_prioridad, gasto.ga_numpagos,gasto.cat_id,cat_description,p.pa_estatus
from pago p 
	...
	IF (NEW.pa_estatus = 2) THEN
        ...
        WHILE _monto>0 DO
            ...
            ELSEIF (_monto = _ingreso) THEN
                ...
                UPDATE gasto set ga_pagoactual = (ga_pagoactual+1), ga_restante=(ga_restante-OLD.pa_monto), ga_status=2 where ga_id = OLD.ga_id;
                SET NEW.pa_fecha_pago = now();
                SET _monto = 0;
            ELSEIF (_monto > _ingreso) THEN 
				UPDATE ingreso set in_restante = 0, in_status = 2 where in_id = _x;
