...
-- usuarios activos
select mes_id,mes_name,count(id) as total from mes
...
BEGIN
	...
	IF (NEW.pa_estatus = 2) THEN
        ...
        IF (_ga_restante<=0) THEN
			UPDATE gasto set ga_status = 2 where ga_id = OLD.ga_id;
        END IF;
...
-- procedimiento almacena para obtener los posibles pagos al registrar un ingreso
drop procedure if exists sp_getPagos;
DELIMITER $$
CREATE PROCEDURE sp_getPagos(in _usu_id int,in _monto decimal(10,2))
BEGIN
    ...
    pa_id int,
    ga_description varchar(50),
    pa_numpago int,
    pa_monto decimal(10,2),
    pa_fecha_pagar date
    );
